...
<div class="container-fluid text-center">    
	<div class="row content">
	    ...
	    <div class="col-sm-6 text-left" style="padding-top:20px;" > <!--* Main form here -->
	     <!--repeat for each row fetched by LoginController -->
	      <c:forEach items="${mypolls}" var="mypoll">
		      <p class = "datecreated">Created on : <c:out value="${mypoll.dateCreated}" /> , Created by : <a href="#"><c:out value="${mypoll.createrName}" /></a></p>
		      <p class = "pollname"><strong><c:out value ="${mypoll.pollname}" /></strong></strong> <span class="badge"><c:out value="${mypoll.avgRating}" /></span></p>
		      <p><c:out value="${mypoll.polldesc}" /></p>
		      <p><a href ="#"><c:out value="${mypoll.weblink}" /></a></p>
		      <a href="TotalReviews?submit=totalReviews&pollid=<c:out value="${mypoll.pollid}"/>"><button id ="totalreviews" class="btn btn-success" type="button">Total Reviews <span class="badge"><c:out value="${mypoll.totalRatings}" /></span></button></a>
		      <a href="TotalReviews?submit=totalReviews&pollid=<c:out value="${mypoll.pollid}"/>"><button id = "writereview" class="btn btn-primary" type="button">Write a Review</button></a>
		      <hr>
	      </c:forEach>
